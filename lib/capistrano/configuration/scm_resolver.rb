  ...
  class Configuration
    ...
    class SCMResolver
      ...
      def warn_set_scm_is_deprecated
        $stderr.puts(<<-MESSAGE)
WARNING: `set :scm, #{scm_name.inspect}` is deprecated.
      ...
      def warn_add_git_to_capfile
        $stderr.puts(<<-MESSAGE)
WARNING: Future versions of Capistrano will no longer load the Git SCM plugin
         automatically. To silence this deprecation warning, add the following
         to your Capfile:
      ...
      def warn_third_party_scm_must_be_upgraded
        $stderr.puts(<<-MESSAGE)
WARNING: Loading the #{scm_name} SCM via `set :scm, #{scm_name.inspect}` is deprecated.
         To ensure this plugin will work with future versions of Capistrano,
         please upgrade it to a newer version. Refer to this post for more info:

         http://capistranorb.com/posts/...
